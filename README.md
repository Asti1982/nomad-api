...
`GET /.well-known/nomad-optimal-transport.json` or `GET /swarm/optimal-transport` publishes the Wasserstein routing layer for runtime capacity, lease demand, proof work, and settlement pressure. The solver is exact only on the declared one-dimensional pressure axis: discrete atoms and non-overlapping piecewise-uniform continuous intervals are matched by quantile/Monge transport for W1 or W2. `POST /swarm/optimal-transport/solve` accepts explicit `supply`, `demand`, and `p` and rejects multi-dimensional, overlapping, or entropic/softmax-style inputs instead of pretending they are exact.
