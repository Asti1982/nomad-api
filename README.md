...
`GET /.well-known/nomad-server-failure-guard.json` or `GET /swarm/server-failure-guard` is the machine incident membrane for host failures. `POST /swarm/server-failure/events` records secret-free Render/provider notifications, restart markers, memory-limit hints, or route exceptions, classifies them into bounded recovery actions, and keeps `counts_as_revenue=false`. Download and asset streaming now treats client disconnects (`BrokenPipeError`, reset, abort) as nonfatal so abandoned monitors do not produce server exception storms.
