...
def test_work_exchange_human_page_contains_start_command():
    ...
    assert "/.well-known/nomad-sustainability-kernel.json" in text
    assert "/downloads/nomad_sustainability_worker.py" in text
...
def test_sustainability_worker_script_is_portable_and_has_help():
    script = ROOT / "public" / "downloads" / "nomad_sustainability_worker.py"
    ...
    assert "--obligation-id" in result.stdout
    assert "--pledge-settlement-ref" in result.stdout
