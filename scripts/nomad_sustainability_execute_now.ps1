...
Write-Step "Public API smoke check"
$checks = @(
    "/health",
    "/openapi.json",
...
if ($workers) {
    ...
    if ($StartLocalWorker) {
        ...
        schtasks /Run /TN "NomadTransitionWorkerRepo-Watchdog" | Out-Null
...
Write-Step "Action focus"
Write-Host "1) If repo/branch != Asti1982/nomad-api main, merge changes there first."
Write-Host "2) Require go_no_go gate: go=true before claiming production readiness."
Write-Host "3) Keep >=1 local worker process alive (loop + swarm_surplus) at all times."
Write-Host "4) Only prioritize lanes with measurable settled outputs."
